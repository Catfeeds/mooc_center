	...
	public function pass_login()
	{
		...
		if (compare_password($user_pass, $user['salt'], $user['user_pass']))
		{
			if ($mode == 2)
			{
                ...
                //登陆token
//                if($terminal == 1){//pc
//                    $data['pc_token']  = generate_token();
//                }else if($terminal == 2){//wx
//                    $data['wx_token']  = generate_token();
//                }else if($terminal == 2){//app
//                    $data['app_token']  = generate_token();
//                }
				...
				else
				{
                   ...
                   /* if($terminal == 1){//pc
                        return ok(['token' => $data['pc_token'], 'salt' => $user['salt'], 'expire_time' => config('mooc_user_expire_time'), 'mooc_user_id' => $user['id']], 25104, '登陆成功', 1);

                    }else if($terminal == 2){//wx
                        return ok(['token' => $data['wx_token'], 'salt' => $user['salt'], 'expire_time' => config('mooc_user_expire_time'), 'mooc_user_id' => $user['id']], 25104, '登陆成功', 1);

                    }else if($terminal == 2){//app
                        return ok(['token' => $data['app_token'], 'salt' => $user['salt'], 'expire_time' => config('mooc_user_expire_time'), 'mooc_user_id' => $user['id']], 25104, '登陆成功', 1);
	...
	private function doUserlogin($center_id, $center_user_id, $param)
	{
		...
		if ($user == NULL)
		{
           ...
           /* if($param['terminal'] == 1){//pc
                $param['pc_token']  = generate_token();
            }else if($param['terminal'] == 2){//wx
                $param['wx_token']  = generate_token();
            }else if($param['terminal'] == 2){//app
                $param['app_token']  = generate_token();
            }*/
