...
class ProxyController extends Controller
{
    ...
    protected function _initialize()
    {
        ...
        $center = \app\v1\model\MoocCenter::get(14);
    ...
    //客户端需要实现获取当前用户昵称的方法
    private function _get_nick_name()
    {
        return '大圣';
    }
...
//        $payload['nick_name'] = $this->_get_nick_name();
        $payload['center_user_id'] = input('param.center_user_id',0,'intval');
        $payload['nick_name'] = input('param.nick_name','','trim');
        $payload['center_id'] = config('proxy.center_id');
        ...
        if ($response['status'] == 1) {
            ...
            return $this->_ok($response, 1101, '登陆成功');
        } else {
            return $response;
        }
    }
    ...
    public function center_login()
    {
        ...
        if ($response['status'] == 1) {
			...
			cookie("mooc_user_id", $response['data']['mooc_user_id'], $response['data']['expire_time']);
