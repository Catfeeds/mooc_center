...
//		$_GET['sign']       = encrypt_key(['v1/user/index', $_GET['timestamp'], $_GET['user_token'], $salt], '');
		...
		$user_login = input('param.nick_name', '', 'string');
		$mode       = input('param.mode', 1, 'intval');  //1前台获取   2 后台获取
...
//			}
		...
		if ( ! empty($nick_name))
		{
			$wehre['nick_name'] = ['like', "%$nick_name%"];
		}
...
//            }
		...
		$data['user_pass']     = generate_password($user_pass, $data['salt']);    //password_hash($password . $salt, PASSWORD_DEFAULT)
		$data['type']          = $type;
	...
	public function read()
	{
		...
		else if ($type == 2)
		{
			...
			$user_info               = $userModel
				...
				->field('u.id,u.profile,u.sex,u.area,u.nick_name,u.teacher_title,u.department,u.user_login,u.avatar,u.type,u.fans_num,u.follow_num')
				->group('u.id')
				->find();
//			print_r($userModel->getLastSql());
//			print_r($user_info);
//			die;
//			$user_info['fans_num'] = (new Follow())->where(['follow_id' => $id])->count(1);


			//判断是否关注服务
			$userRes = verify();

			if ($userRes['code'] == 12000)
			{
				...
				if ($user_info['type'] == 2)
				{
					...
					$classes     = $courseModel
						...
						->field('c.course_title,c.create_time,c.cover_img,c.id,c.start_time,c.end_time,c.open_status,count(b.id) as baoming_num')
						->group('c.id')
				...
				else if ($user_info['type'] == 1)
				{
					...
					$course = (new Baoming())
						...
						->field('c.course_title,c.id,c.cover_img,c.start_time,c.end_time')
						->limit(($page - 1) * $len . ',' . $len)
						->select();
			...
			else
			{
				...
				$Db         = new \think\Db;
				//exit("select * from follow where user_id=" . $user_info['id'] . " and follow_id=" . $id);
				$is_follows = $Db::query("select * from follow where user_id=" . $user_id . " and follow_id=" . $id);
...
//		$user_token = $userModel->where(['id' => $user_id])->value('user_token');
		if ($user_token)
		{
			...
			if ($user_type == 1)
			{
				...
				if (in_array($this->request->param('sex'), array(0, 1, 2)) && ! empty($sql_update))
				{
					$sql_update .= " ,sex=" . $this->request->param('nick_name') . "";
				}
				elseif (in_array($this->request->param('sex'), array(0, 1, 2)) && empty($sql_update))
				{
					$sql_update .= " SET sex=" . $this->request->param('sex') . "";
				}
...
//				$param['user_pass'] = generate_password($user_pass);
