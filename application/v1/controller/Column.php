...
//        $mode = input('param.mode',1,'intval'); //1前台
//
//        //验证token sign
//        if($mode != 1){
//            $msg = verify();
...
//        $len = input('param.len',10,'intval');
        ...
        //获取父级进行分页
        $columnList = $columnModel->alias('c')->join('mooc_center mc','mc.id=c.center_id')->where(['c.parent_id'=>0])->order(['c.center_id asc','c.list_order asc'])->field('c.*,mc.center_name')->select();
//        $num = $columnModel->alias('c')->join('mooc_center mc','mc.id=c.center_id')->where(['c.parent_id'=>0])->order(['c.center_id asc','c.list_order asc'])->count(1);
        ...
        foreach($columnList as $key =>$item){
            ...
            $columnList[$key]['children'] = array_values($columnArray);
        }
...
//        $_GET['center_token'] = 'c72466ee8c653a75467f99c53a58ac252bad9ecc';
//        $_GET['timestamp'] = time();
//        $salt = 'Tt6j0';
//        $_GET['sign'] = encrypt_key(['v1/column/getcolumntable', $_GET['timestamp'], $_GET['center_token'], $salt], '');

//        return $this->request->param('center_token');
        //签名校验
//        $msg = verify();
//        if ($msg['status'] == 0) {
//            return $msg;
...
//        }
    ...
    public function TableTree($center_id){
        ...
        $columnList = $columnModel->alias('c')->join('mooc_center mc','mc.id=c.center_id')->where($where)->order('c.center_id asc')->field('c.*,mc.center_name')->select();
        if ($columnList) {
            $columnList = \collection($columnList)->toArray();
        } else {
            return $this->ok($columnList,11001, '获取栏目成功');
        }
        ...
        return $this->ok($tableTree,11101,'获取栏目成功');
