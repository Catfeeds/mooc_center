...
class SpecialSubject extends Core
{
    ...
    public function index()
    {
        ...
        if($cid != 0) $where['s.center_id'] = $cid;
...
//            $speList[$key]['uploder'] = (new MoocCenter())->where(['id' =>$item['user_id']])->value('center_name');
            ...
            $course_paly_num = (new CenterCourse())->where(['course_id'=>['in',$course],'center_id'=>$center_id])->sum('play_num');
            ...
            $speList[$key]['play_num'] = $course_paly_num;
        }
...
//        }
        ...
        foreach ($spe_mk as $key=>$item) {
            if($item['score'] == null){
                $spe_mk[$key]['score'] = 0;
            }else{
                $spe_mk[$key]['score'] = ceil($item['score']);
            }
        }
