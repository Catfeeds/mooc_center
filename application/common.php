...
function checkUserLogin($user_login, $center_id, $where = [])
{
    if (empty($user_login))
    {
...
//    }
    $type = 2;
    $num  = db('mooc_user')->where(['user_login' => $user_login, 'center_id' => $center_id, 'type' => $type])->where($where)->count(1);
    if ($num > 0)
    {
        return fail(25016, '用户名已存在', 1);
    }
    return TRUE;
}
...
function checkUserToken($user_token){
    $terminal = input('param.terminal',0,'intval');
    if (strlen($user_token) == 0)
    {
        return fail(10000, '令牌不能为空', 1);
    }

    $token = db('token')->where(['user_token'=>$user_token,'type'=>$terminal])->find();
    $user = db('mooc_user')->where('id', $token['user_id'])->find();
    if ($token == NULL)
    {
    ...
    $expire_time = $token['access_time'] + config('mooc_user_expire_time');
    if ($expire_time < time())
    {
...
//    if($param['terminal'] == 1){//pc
//        $param['pc_token']  = input('param.user_token');
//    }else if($param['terminal'] == 2){//wx
//        $param['wx_token']  = input('param.user_token');
//    }else if($param['terminal'] == 2){//app
//        $param['app_token']  = input('param.user_token');
//    }

//    if ( ! empty($param['pc_token']))
//    {
//        //验证签名 token
//        $uToken = checkUserToken($param['pc_token']);
//        if ($uToken !== TRUE)
//        {
//            return $uToken;
//        }
//        $user_info = db('mooc_user')->where(['pc_token' => $param['pc_token']])->find();
//        return ok(['user_id' => $user_info['id'], 'center_id' => $user_info['center_id'], 'type' => 2], 10101, '成功');
//    }
//    if ( ! empty($param['wx_token']))
//    {
//        //验证签名 token
//        $uToken = checkUserToken($param['wx_token']);
//        if ($uToken !== TRUE)
//        {
//            return $uToken;
//        }
//        $user_info = db('mooc_user')->where(['user_token' => $param['wx_token']])->find();
//        return ok(['user_id' => $user_info['id'], 'center_id' => $user_info['center_id'], 'type' => 2], 10101, '成功');
//    }
//    if ( ! empty($param['app_token']))
//    {
//        //验证签名 token
//        $uToken = checkUserToken($param['app_token']);
//        if ($uToken !== TRUE)
//        {
//            return $uToken;
//        }
//        $user_info = db('mooc_user')->where(['user_token' => $param['app_token']])->find();
//        return ok(['user_id' => $user_info['id'], 'center_id' => $user_info['center_id'], 'type' => 2], 10101, '成功');
//    }
//
    if ( ! empty($center_token))
    ...
    if ( ! empty($user_token))
    {
        ...
        $user_info = (new \app\v1\model\MoocUser())->alias('mu')->join('token t','t.user_id=mu.id')->where(['t.user_token'=>$user_token])->field('mu.*')->find();
//        if ($user_info['type'] == 1) {
